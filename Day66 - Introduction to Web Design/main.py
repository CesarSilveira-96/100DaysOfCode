...
def get_all_cafes():
    ...
    all_cafes = result.scalars().all()
    # Simply convert the all_cafes data record to a dictionary of key-value pairs.
    return jsonify(cafes=[cafe.to_dict() for cafe in all_cafes])


@app.route("/search")
def get_location_cafes():
    # result = db.session.execute(db.select(Cafe))
    # all_cafes = result.scalars().all()
    # # Simply convert the all_cafes data record to a dictionary of key-value pairs.
    # return jsonify(cafes=[cafe.to_dict() for cafe in all_cafes if loc.lower() in cafe.location.lower()])
